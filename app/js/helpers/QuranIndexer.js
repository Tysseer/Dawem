...
class QuranIndexer {
  constructor() {
    ...
    this.arrRukuuFirstAyah = []; // the global index of first ayah in each Rukuu
  ...
  getRukuuFromAyah(iAyah /*global index */) {
    iAyah = this.secureIndexRange(iAyah, 6236);

    if (iAyah == 1) return 1;
    if (this.arrRukuuFirstAyah.length == 0) this.fillArrRukuuFirstAyah();
    for (var i = 2; i < this.arrRukuuFirstAyah.length; i++) {
      if (this.arrRukuuFirstAyah[i] > iAyah) {
        return i - 1;
      }
    }
    return 604;
  ...
  getRukuuAyahRange(iRukuu) {
    iRukuu = this.secureIndexRange(iRukuu, this.getNumRukuus());
    if (this.arrRukuuFirstAyah.length == 0) this.fillArrRukuuFirstAyah();
    var start = this.arrRukuuFirstAyah[iRukuu];
    var end =
      iRukuu == this.getNumRukuus() /*last page */
        ? 6236
        : this.arrRukuuFirstAyah[iRukuu + 1] - 1;
    return [start, end];
  }
  ...
  fillArrRukuuFirstAyah() {
    this.arrRukuuFirstAyah = [
      -1, //one based indexes
      1,
      8,
      15,
  ...
  getAutoTitle(strtAyah, endAyah, bIsAr) {
    let indxInforStrt = this.getAyahLocalIndx(strtAyah);
    let indxInforEnd = this.getAyahLocalIndx(endAyah);
    let strtSurah = indxInforStrt.localSurahIndx;
    let endSurah = indxInforEnd.localSurahIndx;
    strtAyah = indxInforStrt.localAyahIndx;
    endAyah = indxInforEnd.localAyahIndx;

    var strStrtA = strtAyah.toString();
    var strEndA = endAyah.toString();
    if (bIsAr) {
      strStrtA = MiscUtilities.convertToIndianNumbers(strStrtA);
      strEndA = MiscUtilities.convertToIndianNumbers(strEndA);
    }
    var strStrt = bIsAr
      ? this.getSurahNameAr(strtSurah)
      : this.getSurahNameEnTrns(strtSurah);
    var strEnd = bIsAr
      ? this.getSurahNameAr(endSurah)
      : this.getSurahNameEnTrns(endSurah);
    if (strStrt == strEnd) {
      //same surah
      if (strtAyah == 1) {
        // starting from beginning of surah
        if (endAyah == this.getSurahNumAyah(endSurah)) {
          // all of the surah
          return strStrt;
        }
      }
